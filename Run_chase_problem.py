...
def fitness(selected_batsmen, avg_runs, target_run):
    total_runs = sum(avg_runs[i] for i in range(len(selected_batsmen)) if selected_batsmen[i])
...
def mutate(selected_batsmen, mutation_rate):
    for i in range(len(selected_batsmen)):
        if random.random() < mutation_rate:
            selected_batsmen[i] = 1 - selected_batsmen[i]  # Flip 0 to 1 or 1 to 0
...
def genetic_algorithm(num_batsmen, avg_runs, target_run, population_size, max_iterations, mutation_rate):
    ...
    for generation in range(max_iterations):
        fitness_scores = [fitness(selected_batsmen, avg_runs, target_run) for selected_batsmen in population]
...
def main():
    ...
    result = genetic_algorithm(num_batsmen, avg_runs, target_run, population_size, max_iterations, mutation_rate)
    ...
    if result is None:
        ...
        for i in range(num_batsmen):
            print(f"{avg_runs[i]} - {'Selected' if result[i] == 1 else 'Not Selected'}")
